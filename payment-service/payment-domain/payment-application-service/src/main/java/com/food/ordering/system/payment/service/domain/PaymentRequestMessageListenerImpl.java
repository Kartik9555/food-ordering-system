...
public class PaymentRequestMessageListenerImpl implements PaymentRequestMessageListener {
    ...
    private void processPayment(Function<PaymentRequest, Boolean> function, PaymentRequest paymentRequest, String methodName) {
        int execution = 1;
        boolean result;
        do {
            log.info("Executing {} operation for {} time for order id {}", methodName, execution, paymentRequest.getOrderId());
            try {
                result = function.apply(paymentRequest);
                execution++;
