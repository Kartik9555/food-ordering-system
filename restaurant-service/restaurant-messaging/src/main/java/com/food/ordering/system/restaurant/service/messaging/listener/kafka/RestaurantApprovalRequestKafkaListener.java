...
import com.food.ordering.system.resatuarant.service.domain.exception.RestaurantNotFoundException;
...
public class RestaurantApprovalRequestKafkaListener implements KafkaConsumer<RestaurantApprovalRequestAvroModel> {
    ...
    public void receive(@Payload List<RestaurantApprovalRequestAvroModel> messages,
        ...
        log.info("{} number of order approval requests received with keys {}, partitions {} and offsets {}, sending for restaurant approval", messages.size(), keys.toString(), partitions.toString(), offsets.toString());
        messages.forEach(restaurantApprovalRequestAvroModel -> {
            try {
                ...
                if (sqlException != null && sqlException.getSQLState() != null && PSQLState.UNIQUE_VIOLATION.getState().equals(sqlException.getSQLState())) {
                    ...
                    log.error("Caught unique constraint exception with sql state: {} in RestaurantApprovalRequestKafkaListener for order id: {}", sqlException.getSQLState(), restaurantApprovalRequestAvroModel.getOrderId());
                } else {
